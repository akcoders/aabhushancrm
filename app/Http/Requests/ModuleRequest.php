...
class ModuleRequest extends FormRequest
{
    ...
    {
        if (in_array($this->route()?->getName(), ['leads.store', 'leads.update', 'followups.store', 'followups.update', 'tasks.store', 'tasks.update'])
    ...
    public function rules(): array
    {
        $rules = match ($this->route()?->getName()) {
            ...
            'followups.store','followups.update' => ['type' => 'required|in:Call,WhatsApp,Visit,Meeting,Email', 'scheduled_at' => 'required|date', 'status' => 'nullable|in:Pending,Completed,Cancelled,Overdue', 'assigned_to' => 'nullable|exists:users,id'],
            'customers.store','customers.update' => ['name' => 'required|string|max:120', 'mobile' => 'required|string|max:30', 'email' => 'nullable|email', 'birthday' => 'nullable|date', 'anniversary' => 'nullable|date', 'city' => 'nullable|string|max:100', 'address' => 'nullable|string', 'notes' => 'nullable|string'],
